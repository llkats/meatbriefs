    ...
    httpRequest.onreadystatechange = function() {
      if (httpRequest.readyState === 4) {
        if (httpRequest.status === 200) {
          ...
          if (newmeats.length < 1) {
            var loading = document.getElementById('loading');
            if (document.contains(loading)) {
              var p = document.createElement('p');
                  p.innerHTML = 'that\'s all the fomo for today, time to go outside or drink';

              loading.getElementsByTagName('img')[0].src = 'data:image/gif;base64,R0lGODlhAQABAIAAAAAAAP///yH5BAEAAAAALAAAAAABAAEAAAIBRAA7';
          ...
          oldlist.removeChild(document.getElementById('loading'));
